...
export class PaymentsService {
  ...
  getCardCheckoutDetails(
    paymentRequestId: string | undefined,
    paymentDetails: PaymentDetails): Promise<PaymentAuthResponse> {
    let consumerId = paymentDetails?.consumerId;
    if (!consumerId) {
      consumerId = uuidv4();
    }
    ...
    const dataToSend = {
      ...
      applicationUserId: consumerId,
      consumerId: paymentDetails?.consumerId || consumerId,
      institutionId: "rapyd",
      taxPercentage: paymentDetails?.taxPercentage ?? 0,
      servicePercentage: paymentDetails?.servicePercentage ?? 0,
